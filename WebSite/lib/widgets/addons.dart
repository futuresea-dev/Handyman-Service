...
listAddons(List<Widget> list, double windowWidth, Function() _redraw, MainModel _mainModel){
  ...
  for (var item in _mainModel.currentService.addon)
    list2.add(Container(
        width: isMobile() ? windowWidth*0.5 : windowWidth*0.2,
        child: Column(
          children: [
            button2addon(getTextByLocale(item.name, strings.locale), item.selected ? theme.style14W800W : theme.style14W600,
                "\$${item.price.toStringAsFixed(0)}", item.selected ? theme.style14W800W : theme.style14W600,
                //
  ...
  if (_mainModel.currentService.addon.isNotEmpty) {
    list.add(Container(
      margin: EdgeInsets.only(left: 10, right: 10),
        child: Text(strings.get(23),/// "Addons",
